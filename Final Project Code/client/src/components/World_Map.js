...
const WorldMap = ({ selectedObservatory, onSelectObservatory }) => {
  ...
  useEffect(() => {
    // Fetch merged geojson (world + observatory points)
  ...
  const drawMap = (geojson) => {
    ...
    // Separate land (Polygons) and observatory points
    ...
    svg.append('g')
      ...
      .attr('r', d => d.properties.name === selectedObservatory ? 6 : 4)
      .attr('fill', d => d.properties.name === selectedObservatory ? '#d62728' : '#1f77b4')
      .attr('stroke', '#fff')
      .attr('stroke-width', 1)
      .style('cursor', 'pointer')
      .on('click', (event, d) => {
        onSelectObservatory(d.properties.name);
      })
      .append('title')
